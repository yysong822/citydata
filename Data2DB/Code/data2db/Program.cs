...
namespace data2db
{
    class Program
    {
        ...
        static String connectionString = "Data Source=YYSONG-PC\\SQLEXPRESS;Initial Catalog=weatherdata;Integrated Security=SSPI;";
        ...
        //指数zu文件
        //20160722 syy增加交通、路况和运动三种指数
        //20160722 syy数据库增加3列，element09交通，element10路况，element11运动指数
        public static void runIndex(string dtype, int[] times, string newFile)
        {
            ...
            //20160722 syy增加交通、路况和运动三种指数
            //20160722 syy数据库增加3列，elm9交通，elm10路况，elm11运动指数
            string elm1, elm2, elm3, elm4, elm5, elm6, elm7, elm8, elm9, elm10, elm11;
                ...
                if (File.Exists(newFile))
                {
                    ...
                    try
                    {
                        ...
                        while (stLine != null && stLine != "")
                        {
                            ...
                            //20160722 syy增加交通、路况和运动三种指数
                            //20160722 syy数据库增加3列，elm9交通，elm10路况，elm11运动指数
                            //20160722 syyelm9交通,数据源中第16列
                            elm9 = Convert.ToInt32(newCF[16].Substring(1, 4)).ToString();
                            //20160722 syyelm10交通,数据源中第17列
                            elm10 = Convert.ToInt32(newCF[17].Substring(1, 4)).ToString();
                            //20160722 syyelm11交通,数据源中第27列
                            elm11 = Convert.ToInt32(newCF[27].Substring(1, 4)).ToString();

                            //sql_citydata = "INSERT INTO tb_AllDataRecorde (StationID, Time, ReportTime, TimeBegin, TimeOver, TimeType, DataType, Element01, Element02, Element03, Element04, Element05, Element06, Element07, Element08) VALUES (" + newCF[0].Trim() + ",'" + sx.ToString() + "','" + reportTime + "','" + timeBegin + "','" + timeOver + "','" + timeType + "','" + dtype + "','" + elm1 + "','" + elm2 + "','" + elm3 + "','" + elm4 + "','" + elm5 + "','" + elm6 + "','" + elm7 + "','" + elm8 + "')";
                            //20160722 syy增加交通、路况和运动三种指数
                            //20160722 syy数据库增加3列，elm9交通，elm10路况，elm11运动指数
                            sql_citydata = "INSERT INTO tb_AllDataRecorde (StationID, Time, ReportTime, "
                                        + "TimeBegin, TimeOver, TimeType, DataType, Element01, Element02, "
                                        + "Element03, Element04, Element05, Element06, Element07, Element08, Element09, Element10, Element11"
                                        + ") VALUES (" + newCF[0].Trim() + ",'" + sx.ToString() + "','"
                                        + reportTime + "','" + timeBegin + "','" + timeOver + "','"
                                        + timeType + "','" + dtype + "','" + elm1 + "','" + elm2 + "','"
                                        + elm3 + "','" + elm4 + "','" + elm5 + "','" + elm6 + "','"
                                        + elm7 + "','" + elm8 + "','" + elm9 + "','" + elm10 + "','" + elm11 + "')";
                            MyCmd.CommandText = sql_citydata;
                            MyCmd.ExecuteNonQuery();
                            Console.WriteLine("插入数据库记录：" + count);
                            count++;
